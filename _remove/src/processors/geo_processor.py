...
# 행정동명 → (lat, lon) 매핑 (시뮬레이션 시각화용)
# 주요 역 좌표를 기반으로 근접 동 이름에 매핑
DONG_CENTROIDS: dict[str, tuple[float, float]] = {
    "강남동": (37.4979, 127.0276),     # 강남역
    "역삼동": (37.5006, 127.0369),     # 역삼역
    "서초동": (37.4916, 127.0076),     # 서초역
    "삼성동": (37.5089, 127.0631),     # 삼성역
    "잠실동": (37.5133, 127.1000),     # 잠실역
    "선릉동": (37.5045, 127.0490),     # 선릉역
    "교대동": (37.4934, 127.0145),     # 교대역
    "방배동": (37.4813, 126.9976),     # 방배역
    "사당동": (37.4764, 126.9816),     # 사당역
    "신림동": (37.4841, 126.9299),     # 신림역
    "구로동": (37.4851, 126.9015),     # 구로디지털단지역
    "가산동": (37.4818, 126.8828),     # 가산디지털단지역
    "홍대동": (37.5571, 126.9241),     # 홍대입구역
    "합정동": (37.5497, 126.9136),     # 합정역
    "망원동": (37.5560, 126.9102),     # 망원역
    "여의도동": (37.5215, 126.9243),   # 여의도역
    "영등포동": (37.5246, 126.8961),   # 영등포구청역
    "신촌동": (37.5553, 126.9369),     # 신촌역
    "종로동": (37.5701, 126.9830),     # 종각역
    "명동": (37.5610, 126.9859),       # 명동역
    "이태원동": (37.5346, 126.9943),   # 이태원역
    "성수동": (37.5446, 127.0558),     # 성수역
    "건대동": (37.5406, 127.0697),     # 건대입구역
    "왕십리동": (37.5614, 127.0382),   # 왕십리역
    "압구정동": (37.5271, 127.0283),   # 압구정역
    "신사동": (37.5164, 127.0206),     # 신사역
    "논현동": (37.5102, 127.0212),     # 논현역
    "청담동": (37.5194, 127.0528),     # 청담역
    "목동": (37.5264, 126.8758),       # 목동역
    "노원동": (37.6554, 127.0613),     # 노원역
    "상계동": (37.6602, 127.0730),     # 상계역
    "미아동": (37.6137, 127.0300),     # 미아역
    "길음동": (37.6037, 127.0250),     # 길음역
    "불광동": (37.6110, 126.9298),     # 불광역
    "연신내동": (37.6189, 126.9214),   # 연신내역
    "공덕동": (37.5440, 126.9518),     # 공덕역
    "마포동": (37.5395, 126.9458),     # 마포역
    "용산동": (37.5547, 126.9707),     # 서울역 (용산 인근)
    "중구동": (37.5637, 126.9770),     # 시청역
    "동대문동": (37.5712, 127.0095),   # 동대문역
    "학동": (37.5149, 127.0315),       # 학동역
}


def dong_name_to_coords(dong_name: str) -> tuple[float, float] | None:
    """동 이름을 좌표(lat, lon)로 변환. 매핑이 없으면 None 반환."""
    return DONG_CENTROIDS.get(dong_name)
