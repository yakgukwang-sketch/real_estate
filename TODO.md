...
- [x] **건물 분류 세분화**: "상점"을 "상점"(의류/잡화) + "식료품점"(슈퍼/편의점/반찬)으로 세분화, sub_categories 추가 (2026-03-24)
  - 상가 API `indsSclsNm`(소분류명) 활용하여 재분류, 대치동+영등포 모두 적용
- [x] **병원/약국 방문 확률 상향**: 모든 프로파일에 병원/약국 방문 동기 추가 (2026-03-24)
  - 약국/의약품소매점 → "병원/약국" 재분류, 괴리 11.1% → 0.1% 달성
- [x] **기타 업종 목적지 추가**: 숙박/유흥 dest_type 신설 (2026-03-24)
- [x] **업종별 객단가 재설정**: 생활서비스 60,900→18,000원, 병원/약국 39,700→25,000원 등 현실화 (2026-03-24)
- [ ] **보정 가중치 적용**: 시뮬레이션 방문 비율을 실제 매출 비중으로 보정

### 대치동 현실 비교 (2026-03-26 진행)
- [x] 대치동 시뮬레이션 vs 실제 spending 데이터 비교 실행 (2026-03-26)
  - 결과: 시뮬레이션 81% vs 실제 52% — 학원비 제외 구조적 차이
  - 학원 객단가 0원 설정으로 상점 매출만 시뮬레이션하도록 변경
- [x] 영등포역 시뮬레이션 정확도 재확인: 10.8% 과소추정 (양호) (2026-03-26)
- [ ] 대치/은마/도곡 상권 실제 업종별 매출 추출 (학원 제외 기준 재정의 필요)

### 2026-03-26 발견 개선사항 (미커밋 작업 완료, 커밋 필요)
- [x] 대치동 다중 출발점(origin_points) 추가 — sidewalk.py DAECHI_CONFIG (2026-03-26)
  - 학원학생 30%, 외부직장인 18%, 방문객 12% / 기존 주민 프로파일 40%로 축소
- [x] 시간대별 객단가 차등 적용 — 저녁 x1.8, 회식 x2.5 (2026-03-26)
- [x] 학원 객단가 0원 설정 (상점 매출만 시뮬레이션) (2026-03-26)
- [x] 상세 이동 경로 분석 스크립트 작성 — scripts/detailed_movement.py (2026-03-26)
- [x] SIMULATION.md 전체 업데이트 (2026-03-26)

### 다음 개선 과제 (2026-03-27~)
- [ ] 도보 20분+ 이동 33% 과다 문제 — 네트워크 단절 우회 경로 보정 로직 추가
- [ ] 식료품점/편의점 방문 확률 과소 — 프로파일별 방문 동기 비중 재조정
- [ ] 영등포 건물명 누락 데이터 보완
- [ ] 요일별 패턴 반영 (평일/주말 분리, 대치동 주말 학원가 별도 처리)
- [ ] 연쇄 이동(trip chain) 비중 향상
- [ ] 배달/온라인 매출 보정 모델 추가 (중장기)
